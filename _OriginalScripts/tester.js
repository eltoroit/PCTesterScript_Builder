...
const bmChromePath =
  "C:\\Users\\Dell-7480\\AppData\\Local\\Google\\Chrome\\User Data\\Profile 1\\Bookmarks";
const bmFirefoxPath = [
...
function spawnCommand(instruction) {
  instruction.Command__c = JSON.parse(
    instruction.Command__c.replace(/\\/g, "\\\\")
  );
  if (debug)
    log.debug("SPAWNING: " + log.getPrettyJson(instruction.Command__c));

  var process;
  try {
    if (instruction.Command__c.params && instruction.Command__c.params !== "") {
      process = spawn(
        instruction.Command__c.cmd,
        instruction.Command__c.params
      );
    } else {
...
function checkContains(instruction) {
  ...
  instruction.callback = function(output) {
    ...
    if (isExecute) {
      ...
      if (
        output.stdout != "" &&
        output.stdout.indexOf(instruction.Expected__c) >= 0
      )
        valid = true;
      if (
        output.stderr != "" &&
        output.stderr.indexOf(instruction.Expected__c) >= 0
      )
        valid = true;
    }
  ...
  if (!instruction.callback) {
    instruction.callback = function(output) {
      if (
        output.stdout
          .toLowerCase()
          .indexOf(instruction.Expected__c.toLowerCase()) >= 0
      ) {
        if (verbose)
          log.success("VALID: [Found: '" + instruction.Expected__c + "']");
        nextInstruction();
      } else {
        instruction.returned = output;
        reportError(instruction);
...
function openUrl(urlToCheck, callback) {
  if (checkUrlExists) {
    ...
    if (debug)
      log.debug(
        JSON.stringify(process.stdout.toString("utf8")).substring(0, 250)
      );
...
function findBookmarks_Firefox() {
  ...
  if (debug)
    log.debug(
      `[Firefox Bookmarks][LOLG]: Searching for Firefox bookmars at path: ${sqlitepath}`
    );
  var files = fs.readdirSync(sqlitepath);
  if (debug)
    log.debug(
      `[Firefox Bookmarks][LOLG]: Files found: ${JSON.stringify(files)}: `
    );
  if (files.length == 1) {
    sqlitepath += "\\" + files[0] + "\\" + bmFirefoxPath[2];
    if (debug)
      log.debug(`[Firefox Bookmarks][OK]: Full bookmars path: ${sqlitepath}`);
  } else {
    ...
    lineReader.on("line", function(line) {
      if (line == "") {
        ...
        if (tmp.TitlesByRow[record.id]) {
          reportErrorMessage(
            "Searching for Firefox bookmars: *" +
              record.id +
              "* was already defined"
          );
        } else {
        ...
        if (record.bTitle) {
          ...
          if (tmp.TitlesByName[title]) {
            reportErrorMessage(
              "Searching for Firefox bookmars: Duplicate record: [" +
                record.bTitle +
                "]"
            );
          }
    ...
    lineReader.on("close", function() {
      ...
      for (var path in bmBarTemp) {
        if (bmBarTemp.hasOwnProperty(path)) {
          ...
          nodeNew.urlExpected =
            nodeNew.urlChrome === nodeNew.urlFirefox
              ? nodeNew.urlChrome
              : "NO_IDEA";
...
function jsonFile_FindPath(instruction) {
  ...
  if (JSON_Action.totalSize !== 1)
    throw new Error("Multiple JSON Actions are not allowed!");
...
function jsonFile_Edit(instruction) {
  ...
  fs.writeFile(
    instruction.Command__c,
    log.getPrettyJson(fileContents),
    function(err) {
      ...
      if (err) {
        ...
        if (verbose)
          log.success(
            "VALID: file has been updated: " + instruction.Command__c
          );
        nextInstruction();
      }
    }
...
function executeInstruction() {
  ...
  switch (instruction.Operation__c) {
    ...
    case "Write":
      // Force debug mode...
      if (
        instruction.Command__c == "=== === === AUTOMATED CHECKS === === ==="
      ) {
    ...
    case "Done":
      ...
      try {
        ...
        if (debug)
          log.debug(
            "Could not delete file " + filePath + ": " + log.getPrettyJson(ex)
          );
      }
      if (errors.length > 0) {
        log.clearScreen();
        ...
        log.clearScreen();
        log.success(
          "Test complete and no errors found. Thanks for your help ;-)"
        );
        log.success(
          "Please close this and all other windows that were opened during the test"
        );
      }
      process.exit(0);
      break;
...
function menuChooseEvent(data) {
  ...
  log.setDebug(debug);
  log.info(
    "Application Tester built by Andres Perez (ELTORO.IT) to help validate the computer's setup"
  );
  log.info("");
  ...
  var forEver = function() {
    inputReadLine2.question(
      log.getPromptMsg("Please select a number [0 - " + events.length + "] > "),
      function(answer) {
        if (answer == 0) {
          process.exit(0);
...
function bookmarksChecks() {
  ...
  if (doesFileExist(bmCheckPath)) {
    ...
    if (!(bmChecks.length > 0)) {
      ...
      var msg =
        "Invalid bookmarks checker configuration file [" + bmCheckPath + "]!";
      log.error(msg);
      throw new Error(msg);
    }
  } else {
    log.error("BOOKMARKS CAN NOT PROCESSED!!!");
    var msg =
      "Bookmarks checker configuration file [" +
      bmCheckPath +
      "] does not exist!";
    log.error(msg);
    throw new Error(msg);
