...
const Logger = {
	...
	Logger: class {
		...
		prepareLogs(desiredLogLevel) {
			const that = this;

			return new Promise((resolve, reject) => {
				that.getFileStream(config.logger.levels[desiredLogLevel] <= config.logger.levels.info).on("open", fd => {
		...
		_add(logLevel, name, lineNumber, message, tooMany) {
			...
			if (config.logger.levels[config.debug.mode] <= logLevel) {
				...
				msg += `${data.message}\t`;
				that.getFileStream(config.logger.levels[config.debug.mode] <= config.logger.levels.info).write(`${msg}\n`);
		...
		getFileStream(withMinutes) {
			// Get file name which is based on a timestamp
			const fileName = this.getFileName(withMinutes);
		...
		getFileName(withMinutes) {
			let fileName = "";
			const now = new Date();

			fileName += `${now.getFullYear()}`;
			fileName += `${String(now.getMonth() + 1).padStart(2, "0")}`;
			fileName += `${String(now.getDate()).padStart(2, "0")}`;
			fileName += `T${String(now.getHours()).padStart(2, "0")}`;
			if (withMinutes) {
				fileName += `${String(now.getMinutes()).padStart(2, "0")}`;
				// fileName += `${String(now.getSeconds()).padStart(2, "0")}`;
			}
